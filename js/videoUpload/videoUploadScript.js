$(function(){
	...
	var process_url = 'http://tellus.uk.com/hellodaisy/upload.php'; //PHP script
	...
	//initialize blueimp fileupload plugin
		...
		dropZone: $('#dropzone')
			...
			data.context = $('<div/>').addClass('file-wrapper col-xs-12').appendTo('#files');
			$.each(data.files, function (index, file){
			var node = $('<div/>').addClass('file-row text-center center-block');
			var removeBtn  = $('<button/>').addClass('button btn-red remove').text('Remove');
			removeBtn.on('click', function(e, data){
				$(this).parent().parent().remove();
			});

			var file_txt = $('<div/>').addClass('clearfix file-row-text center-block').append('<span>'+file.name + ' (' +format_size(file.size) + ')' + '</span>');
